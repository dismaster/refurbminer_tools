...
if [ "$SHOW_HELP" = true ]; then
    ...
    echo "  -restore=MINER_ID      Restore config and set minerId (e.g., -restore=1234567890abcdef)"
    echo "  --restore=MINER_ID     Restore config and set minerId (alternative format)"
    echo "  -h, --help       Show this help message"
    echo
    echo "Examples:"
    echo "  $0                                    # Interactive installation"
    echo "  $0 -token=xyz9876543210abcdef34       # Automated installation with token"
    echo "  $0 -restore=1234567890abcdef         # Restore config and set minerId"
    echo
    exit 0
fi
...
# === Restore mode: update minerId in config.json and exit (AFTER download) ===
if [ "$RESTORE_MODE" = true ]; then
    if [ -z "$RIG_TOKEN" ]; then
        error "Restore mode requires a token as well. Please provide -token=YOUR_TOKEN or --token=YOUR_TOKEN."
        exit 1
    fi
    CONFIG_PATH="$INSTALL_DIR/config/config.json"
    if [ ! -f "$CONFIG_PATH" ]; then
        error "$CONFIG_PATH not found. Cannot restore minerId."
        exit 1
    fi
    if [ -z "$RESTORE_MINER_ID" ]; then
        error "No minerId provided for --restore."
        exit 1
    fi
    # Use jq if available, else fallback to sed
    if command -v jq &>/dev/null; then
        tmpfile=$(mktemp)
        jq --arg id "$RESTORE_MINER_ID" '.minerId = $id' "$CONFIG_PATH" > "$tmpfile" && mv "$tmpfile" "$CONFIG_PATH"
        success "Updated minerId in $CONFIG_PATH to $RESTORE_MINER_ID (using jq)"
    else
        sed -i "s/\("minerId"[[:space:]]*:[[:space:]]*\)\"[^\"]*\"/\1\"$RESTORE_MINER_ID\"/" "$CONFIG_PATH"
        success "Updated minerId in $CONFIG_PATH to $RESTORE_MINER_ID (using sed)"
    fi
    success "Restore complete. Exiting as requested."
    exit 0
fi
